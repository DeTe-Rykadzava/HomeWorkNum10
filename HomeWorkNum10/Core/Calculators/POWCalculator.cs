...
public class POWCalculator : ACalculator
{
    public POWCalculator(string supportedOperation) : base(supportedOperation, UserInputType.Double, 2) { }
    ...
    public override CalculatorResult ProcessOperation(List<dynamic> operators)
    {
        ...
        var result = Math.Pow(operators[0], operators[1]);
        return new CalculatorResult(result, $"{operators[0]} POW {operators[1]} = {result}");
    }
}